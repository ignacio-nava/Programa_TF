...
class Panel_Recinto ( wx.Panel ):
    def __init__( self, parent, id = wx.ID_ANY, pos = wx.DefaultPosition, size = wx.Size( 292,631 ), style = wx.TAB_TRAVERSAL, name = wx.EmptyString ):
        ...
        sizer_U_LEFT = wx.BoxSizer( wx.HORIZONTAL )

        self.boton_restar = wx.BitmapButton( self, wx.ID_ANY, wx.NullBitmap, wx.DefaultPosition, wx.DefaultSize, wx.BU_AUTODRAW|0 )

        self.boton_restar.SetBitmap( wx.ArtProvider.GetBitmap( wx.ART_MINUS,  ) )
        sizer_U_LEFT.Add( self.boton_restar, 0, wx.ALL, 5 )

        self.boton_sumar = wx.BitmapButton( self, wx.ID_ANY, wx.NullBitmap, wx.DefaultPosition, wx.DefaultSize, wx.BU_AUTODRAW|0 )

        self.boton_sumar.SetBitmap( wx.ArtProvider.GetBitmap( wx.ART_PLUS,  ) )
        sizer_U_LEFT.Add( self.boton_sumar, 0, wx.ALL, 5 )

        sizer_UP.Add( sizer_U_LEFT, 0, 0, 5 )

        self.m_staticline2 = wx.StaticLine( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.LI_VERTICAL )
        sizer_UP.Add( self.m_staticline2, 0, wx.EXPAND |wx.ALL, 5 )

        sizer_U_RIGHT = wx.BoxSizer( wx.VERTICAL )

        self.boton_reiniciar = wx.Button( self, wx.ID_ANY, u"Reinciar Recinto", wx.DefaultPosition, wx.DefaultSize, 0 )
        sizer_U_RIGHT.Add( self.boton_reiniciar, 1, wx.ALL|wx.EXPAND, 5 )


        sizer_UP.Add( sizer_U_RIGHT, 1, wx.EXPAND, 5 )
        ...
        self.treeCtrl_recinto = wx.TreeCtrl( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TR_DEFAULT_STYLE|wx.TR_EDIT_LABELS )
        bSizer15.Add( self.treeCtrl_recinto, 2, wx.ALL|wx.EXPAND, 5 )


        sizer_DOWN.Add( bSizer15, 3, wx.EXPAND, 5 )

        self.m_staticline21 = wx.StaticLine( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.LI_HORIZONTAL )
        sizer_DOWN.Add( self.m_staticline21, 0, wx.EXPAND |wx.ALL, 5 )
        ...
        bSizer17.Add( self.staticText_superficie, 1, wx.ALL|wx.EXPAND, 5 )
        ...
        self.grilla_vertices = wx.grid.Grid( self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, 0 )

        # Grid
        self.grilla_vertices.CreateGrid( 4, 3 )
        self.grilla_vertices.EnableEditing( True )
        self.grilla_vertices.EnableGridLines( True )
        self.grilla_vertices.EnableDragGridSize( False )
        self.grilla_vertices.SetMargins( 0, 0 )

        # Columns
        self.grilla_vertices.EnableDragColMove( False )
        self.grilla_vertices.EnableDragColSize( True )
        self.grilla_vertices.SetColLabelSize( 25 )
        self.grilla_vertices.SetColLabelValue( 0, u"X" )
        self.grilla_vertices.SetColLabelValue( 1, u"Y" )
        self.grilla_vertices.SetColLabelValue( 2, u"Z" )
        self.grilla_vertices.SetColLabelAlignment( wx.ALIGN_CENTER, wx.ALIGN_CENTER )

        # Rows
        self.grilla_vertices.EnableDragRowSize( True )
        self.grilla_vertices.SetRowLabelSize( 30 )
        self.grilla_vertices.SetRowLabelAlignment( wx.ALIGN_CENTER, wx.ALIGN_CENTER )

        # Label Appearance

        # Cell Defaults
        self.grilla_vertices.SetDefaultCellAlignment( wx.ALIGN_CENTER, wx.ALIGN_CENTER )
        bSizer18.Add( self.grilla_vertices, 0, wx.ALL, 5 )


        bSizer16.Add( bSizer18, 1, wx.EXPAND, 5 )


        sizer_DOWN.Add( bSizer16, 1, wx.EXPAND, 5 )
 ...
 #      Connect Events
        self.boton_restar.Bind( wx.EVT_BUTTON, self.onRestarSuperficie )
        self.boton_sumar.Bind( wx.EVT_BUTTON, self.onSumarSuperficie )
        #self.grilla_vertices.Bind( wx.EVT_SIZE, self.onSize )
 #      Funciones
    def __del__( self ):
        pass
    def onRestarSuperficie(self,event):
        a_eleminar = self.treeCtrl_recinto.GetSelection()
        if a_eleminar != self.ROOT:
            self.treeCtrl_recinto.Delete(a_eleminar)
        else:
            event.Skip()
    def onSumarSuperficie(self,event):
        padre = self.treeCtrl_recinto.GetSelection()
        n = self.treeCtrl_recinto.GetChildrenCount(padre,recursively=False)
        self.treeCtrl_recinto.AppendItem(padre,'Superficie %s'%(n+1))
        self.treeCtrl_recinto.Expand(padre)
